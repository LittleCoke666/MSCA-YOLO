...
from ultralytics.utils import DEFAULT_CFG, LOGGER, RANK, TQDM, clean_url, colorstr, emojis, callbacks, \
    __version__
...
class DetectionDistiller(BaseTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        try:
            ...
            raise RuntimeError(emojis(f"Dataset '{clean_url(self.args.data)}' error L {e}")) from e
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        if weights:
            ...
            if isinstance(weights, str) and weights.endswith('.pt'):
                ...
                weights = ckpt.get('model', ckpt)  # 获取模型权重
                if hasattr(weights, 'yaml'):
                    model.yaml = weights.yaml  # 传递yaml配置
            model.load(weights)
        return model
    ...
    def progress_string(self):
        ...
        return ('\n' + '%11s' *
                (4 + len(self.loss_names) + 2)) % ('Epoch', 'GPU_mem', *self.loss_names, 'log_loss', 'fea_loss',
                                                   'Instances', 'Size')
    ...
    def _setup_train(self, world_size):
        ...
        if RANK in (-1, 0):
            self.test_loader = self.get_dataloader(self.data.get("val"), batch_size=batch_size * 2, rank=-1, mode='val')
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    self.loss = (main_loss + (log_distill_loss + fea_distill_loss) * batch['img'].size(0) * distill_decay).mean()
