    ...
    if solution_name == "inference":
        ...
        if solution_name != "crop":
            ...
            if solution_name == "analytics":  # analytical graphs follow fixed shape for output i.e w=1920, h=1080
                w, h = 1280, 720
            save_dir = get_save_dir(SimpleNamespace(project="runs/solutions", name="YOLO11n_C3k2_ContextGuidedBlock(YOLO11m_distill)", exist_ok=False))
            save_dir.mkdir(parents=True)  # create the output directory i.e. runs/solutions/YOLO11n_C3k2_ContextGuidedBlock(YOLO11m_distill)
